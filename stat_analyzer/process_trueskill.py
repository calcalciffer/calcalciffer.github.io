...
class TrueSkillCalculator:
    ...
    def get_player_stats_db(self, match, player, player_new_stats: StatModel, delta_value_name: str) -> Dict[str, Any]:
        ...
        player_stats_db[f"lastModified"]["$date"] = str(datetime(2026, 2, 16, 0, 0, 0, tzinfo=UTC))
        if player.leader:
    ...
    def process_ts(self, match_parse_model: MatchParseModel):
        for match in match_parse_model.matches:
            ...
            if match.is_cloud is True:
                ...
                match, post = self.update_player_stats(match, player_ratings, "combined_delta")
                if match is None:
                    continue
                for i, player in enumerate(match.players):
                    player_stats_db = self.get_player_stats_db(match, player, post[i], "combined_delta")
