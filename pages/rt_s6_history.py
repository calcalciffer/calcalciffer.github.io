...
def generate_realtime_history_content():
    for match in reversed(rt_season6_stats.matches_list):
        with div(cls=f"row {match[0].gametype}"), div(cls="chart"):
            ...
            for player in match[0].players:
                ...
                p(f"[ {'+' if player.delta > 0 else ''} {round(player.delta, 2)} ] {get_player_name(player.id['$numberLong'], rt_season6_stats.player_id_name_map)} {get_civ_name(player)}{flags}",
                  style="text-align:left",
                  cls='civ-ability-desc')

def get_realtime_s6_history_page(pages_list, menu_list):
    return create_page(
        title='Season 6 Realtime Game History - Civilization Player League',
        header='realtime_s6_games',
